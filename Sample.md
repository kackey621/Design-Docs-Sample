...
This document describes the sample design documents provided as reference examples. All samples are based on the **Recerdo** service (Viejo App) — a social media platform for reconnecting with old friends and close-knit groups.
...
This sample documents the **User Relationship Service**, a core microservice in Recerdo that manages friend connections, group memberships, and relationship history. It demonstrates how to fill in every section of the microservice template with realistic content including API endpoints, data schemas, event definitions, and deployment configuration.
...
This sample documents the **Memory Sharing Module**, which allows Recerdo users to share photos, stories, and memories within their friend groups. It demonstrates clean architecture layer separation with concrete code examples.
...
Each sample follows the same structure as its corresponding template. Sections that would be `[placeholder]` in the template are filled with Recerdo-specific content to show:
